    ...
    where T : IsoRepoElement
{
    ...
    protected void WriteClassComments(IsoRepoElement item, TextWriter textWriter, int indent=0, params string[] additionalComments)
    {
        ...
        WriteLines(item, textWriter, indent, cleanedUpLines);
        // foreach (var line in item.Definition) textWriter.WriteLine($"{scoot}/// {line}");
    ...
    protected void WriteLines(IsoRepoElement item, TextWriter textWriter, int indent, params string[] linesToWrite)
    {
        var scoot = "".PadLeft(indent, ' ');
