//
// GENERATED CODE, DO NOT EDIT THIS FILE!
//
// Generated by ConcreteChoiceGenerator for PointsOfPutAmount.  ISO2002 ID# _TMLmI9p-Ed-ak6NoX_4Aeg_-1551054603.
//

using BeneficialStrategies.Iso20022.Components;
using BeneficialStrategies.Iso20022.ExternalSchema;
using System.Xml;
using System.Xml.Linq;

namespace BeneficialStrategies.Iso20022.Choices.PremiumQuote1Choice;

/// <summary>
/// Premium calculation is based on points of the put amount.
/// </summary>
public partial record PointsOfPutAmount : PremiumQuote1Choice_
     , IIsoXmlSerilizable<PointsOfPutAmount>
{
    #nullable enable
    
    /// <summary>
    /// Contains the main value for the container.
    /// Rate expressed as a decimal, for example, 0.7 is 7/10 and 70%.
    /// </summary>
    public required IsoBaseOneRate Value { get; init; } 
    
    #nullable disable
    
    
    /// <summary>
    /// Used to format the various primative types during serialization.
    /// </summary>
    public static SerializationFormatter SerializationFormatter { get; set; } = SerializationFormatter.GlobalInstance;
    
    /// <summary>
    /// Serializes the state of this record according to Iso20022 specifications.
    /// </summary>
    public override void Serialize(XmlWriter writer, string xmlNamespace)
    {
        writer.WriteStartElement(null, "PtsOfPutAmt", xmlNamespace );
        writer.WriteValue(SerializationFormatter.IsoBaseOneRate(Value)); // data type BaseOneRate System.Decimal
        writer.WriteEndElement();
    }
    public static new PointsOfPutAmount Deserialize(XElement element)
    {
        throw new NotImplementedException();
    }
}
