...
public partial record ContractRegistrationStatement1
     : IIsoXmlSerilizable<ContractRegistrationStatement1>
{
    #nullable enable
    ...
    public void Serialize(XmlWriter writer, string xmlNamespace)
    {
        ...
        writer.WriteEndElement();
        if (TransactionJournal is TransactionCertificate1 TransactionJournalValue)
        {
            writer.WriteStartElement(null, "TxJrnl", xmlNamespace );
            TransactionJournalValue.Serialize(writer, xmlNamespace);
            writer.WriteEndElement();
        }
        if (SupportingDocumentJournal is SupportingDocument1 SupportingDocumentJournalValue)
        {
            writer.WriteStartElement(null, "SpprtgDocJrnl", xmlNamespace );
            SupportingDocumentJournalValue.Serialize(writer, xmlNamespace);
            writer.WriteEndElement();
        }
        if (AdditionalSupportingDocumentJournal is SupportingDocument1 AdditionalSupportingDocumentJournalValue)
        {
    ...
    public static ContractRegistrationStatement1 Deserialize(XElement element)
    {
